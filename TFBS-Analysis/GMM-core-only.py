...
X = data.loc[data["annotation"] != "exonic", "phylop-core"].values.reshape(-1, 1)
print(X.shape)
print(X)
...
data.loc[(data["phylop-core"] > thresholds[0]), "constrained"] = True

probs = M_best.predict_proba(data["phylop-core"].values.reshape(-1, 1))
print(probs.shape)
print(probs[0,:])
data["p"] = probs[:,1]
data.loc[data["phylop-core"] < 0, "p"] = 0
